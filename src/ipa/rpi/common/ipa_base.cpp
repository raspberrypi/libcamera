...
void IpaBase::reportMetadata(unsigned int ipaContext)
{
	...
	if (cnnEnableInputTensor_ && inputTensor) {
		...
		Span<const uint8_t> tensor{ (const uint8_t *)inputTensor->get(), size };
	...
	if (inputTensorInfo) {
		Span<const uint8_t> tensorInfo{ (const uint8_t *)inputTensorInfo, sizeof(*inputTensorInfo) };
	...
	if (outputTensor) {
		...
		Span<const float> tensor{ (const float *)outputTensor->get(), size };
	...
	if (outputTensorInfo) {
		Span<const uint8_t> tensorInfo{ (const uint8_t *)outputTensorInfo, sizeof(*outputTensorInfo) };
	...
	if (kpiInfo) {
		Span<const uint8_t> kpi{ (const uint8_t *)kpiInfo, sizeof(*kpiInfo) };
		libcameraMetadata_.set(controls::rpi::CnnKpiInfo, kpi);
	}
