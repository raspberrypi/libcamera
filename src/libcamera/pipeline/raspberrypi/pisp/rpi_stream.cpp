...
int Stream::queueBuffer(FrameBuffer *buffer)
{
	...
	if (!buffer) {
		if (availableBuffers_.empty()) {
			LOG(PISPSTREAM, Info) << "No buffers available for "
					     << name_;
			/*
...
{
	LOG(PISPSTREAM, Debug) << "Queuing buffer " << getBufferId(buffer)
			      << " for " << name_;

	int ret = dev_->queueBuffer(buffer);
	if (ret)
		LOG(PISPSTREAM, Error) << "Failed to queue buffer for "
				      << name_;
	return ret;
}
