...
public:
	...
	int init(const IPASettings &settings, ipa::vc4::SensorConfig *sensorConfig) override;
	void start(const ControlList &controls, ipa::vc4::StartConfig *startConfig) override;
	void stop() override {}

	int configure(const IPACameraSensorInfo &sensorInfo,
		      const std::map<unsigned int, IPAStream> &streamConfig,
		      const std::map<unsigned int, ControlInfoMap> &entityControls,
		      const ipa::vc4::IPAConfig &data,
		      ControlList *controls) override;
	void mapBuffers(const std::vector<IPABuffer> &buffers) override;
	void unmapBuffers(const std::vector<unsigned int> &ids) override;
	void signalStatReady(const uint32_t bufferId) override;
	void signalQueueRequest(const ControlList &controls) override;
	void signalIspPrepare(const ipa::vc4::ISPConfig &data) override;
...
private:
	...
	void prepareISP(const ipa::vc4::ISPConfig &data);
	void reportMetadata();
		      ...
		      ControlList *controls)
{
	...
	if (ipaConfig.lsTableHandle.isValid()) {
		...
		if (lsTableHandle_.isValid()) {
			lsTable_ = mmap(nullptr, ipa::vc4::MaxLsGridSize, PROT_READ | PROT_WRITE,
...
void IPARPi::applyLS(const struct AlscStatus *lsStatus, ControlList &ctrls)
{
	...
	if (!lsTable_ || w * h * 4 * sizeof(uint16_t) > ipa::vc4::MaxLsGridSize) {
