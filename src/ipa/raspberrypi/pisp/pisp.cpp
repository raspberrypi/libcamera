			  ...
			  bool isSigned = false, const char *desc = nullptr)
{
	...
	int32_t val = std::clamp<int32_t>(std::round(value * (1 << fracBits)), min, max);

	if (desc && val / (1 << fracBits) != value)
		LOG(IPARPI, Warning) << desc << " rounded/clamped to " << val / (1 << fracBits);
		...
		unsigned int SlopeBits = 14, unsigned int PosBits = 16)
{
	...
	for (unsigned int i = 0; i < lutSize; i++) {
		...
		y = pwl.eval(x);
		if (y < 0 || (i && y < lastY)) {
			LOG(IPARPI, Error) << "Malformed PWL for Gamma, disabling!";
			return -1;
		}
		...
		if (i) {
			...
			if (slope >= (1u << SlopeBits)) {
				...
				LOG(IPARPI, Info) << ("Maximum Gamma slope exceeded, adjusting!");
				y = lastY + slope;
			}
 ...
 * corner sampling.
 */
void resampleTable(double *dest, int destW, int destH, double const *src, int srcW, int srcH)
{
	/*
...
private:
	void prepareIsp(const PrepareParams &params, RPiController::Metadata &rpiMetadata) override;
	RPiController::StatisticsPtr processStats(Span<uint8_t> mem) override;
	void handleControls(const ControlList &controls) override;
	void applyWBG(const AwbStatus *awbStatus, const AgcStatus *agcStatus, pisp_be_global_config &global);
	void applyContrast(const ContrastStatus *contrastStatus, pisp_be_global_config &global);
	void applyCCM(const CcmStatus *ccmStatus, pisp_be_global_config &global);
	void applyBlackLevel(const BlackLevelStatus *blackLevelStatus, pisp_be_global_config &global);
	void applyLensShading(const AlscStatus *alscStatus, pisp_be_global_config &global);
	...
	void applyTdn(const TdnStatus *tdnStatus, const DeviceStatus *deviceStatus, pisp_be_global_config &global);
	...
	void applySaturation(const SaturationStatus *geqStatus, pisp_be_global_config &global);
	void applySharpen(const SharpenStatus *sharpenStatus, pisp_be_global_config &global);
	void applyHdr(const HdrStatus *hdrStatus, const DeviceStatus *deviceStatus, pisp_be_global_config &global);
	void applyTonemap(const TonemapStatus *tonemapStatus, pisp_be_global_config &global);
...
int32_t IpaPiSP::init(const IPASettings &settings, const InitParams &params, InitResult *result)
{
	int ret = IpaBase::init(settings, params, result);
	if (ret)
		return ret;
	...
	fe_ = static_cast<FrontEnd *>(mmap(nullptr, sizeof(FrontEnd), PROT_READ | PROT_WRITE, MAP_SHARED, feFD_.get(), 0));
	be_ = static_cast<BackEnd *>(mmap(nullptr, sizeof(BackEnd), PROT_READ | PROT_WRITE, MAP_SHARED, beFD_.get(), 0));
...
void IpaPiSP::start(const ControlList &controls, StartResult *result)
{
	IpaBase::start(controls, result);

	tdnReset_ = true;
	hdrReset_ = true;
}

int IpaPiSP::configure(const IPACameraSensorInfo &sensorInfo, const ConfigParams &params,
		       ConfigResult *result)
{
	int ret = IpaBase::configure(sensorInfo, params, result);
	if (ret)
		return ret;
...
RPiController::StatisticsPtr IpaPiSP::processStats(Span<uint8_t> mem)
{
	...
	StatisticsPtr statistics =
		std::make_unique<Statistics>(Statistics::AgcStatsPos::PostWb, Statistics::ColourStatsPos::PreLsc);

	/* RGB histograms are not used, so do not populate them. */
	statistics->yHist = std::move(RPiController::Histogram(stats->agc.histogram, PISP_AGC_STATS_NUM_BINS));
	...
	for (i = 0; i < statistics->agcRegions.numRegions(); i++)
		statistics->agcRegions.setFloating(i,  { { 0, 0, 0, stats->agc.floating[i].Y_sum },
...
void IpaPiSP::prepareIsp([[maybe_unused]] const PrepareParams &params,
			 RPiController::Metadata &rpiMetadata)
{
	...
	global.bayer_enables &= ~(PISP_BE_BAYER_ENABLE_BLC + PISP_BE_BAYER_ENABLE_WBG + PISP_BE_BAYER_ENABLE_GEQ +
				  PISP_BE_BAYER_ENABLE_LSC + PISP_BE_BAYER_ENABLE_SDN + PISP_BE_BAYER_ENABLE_CDN +
	...
	global.rgb_enables &= ~(PISP_BE_RGB_ENABLE_GAMMA + PISP_BE_RGB_ENABLE_CCM + PISP_BE_RGB_ENABLE_YCBCR +
				PISP_BE_RGB_ENABLE_YCBCR_INVERSE + PISP_BE_RGB_ENABLE_SHARPEN +
				PISP_BE_RGB_ENABLE_SAT_CONTROL);
...

	{
		...
		if (awbStatus)
			applyWBG(awbStatus, agcStatus, global);
	}
	...
	SaturationStatus *saturationStatus = rpiMetadata.getLocked<SaturationStatus>("saturation.status");
	if (saturationStatus)
...
void IpaPiSP::applyWBG(const AwbStatus *awbStatus, const AgcStatus *agcStatus, pisp_be_global_config &global)
{
	pisp_wbg_config wbg;
	pisp_fe_rgby_config rgby = {};
	double dg = agcStatus ? agcStatus->digitalGain : 1.0;
	...
	/*
	 * The YCbCr conversion block should contain the appropriate YCbCr matrix.
	 * We should not rely on the CSC0 block as that might be programmed for RGB outputs.
	 */
...
void IpaPiSP::applyContrast(const ContrastStatus *contrastStatus, pisp_be_global_config &global)
{
...
void IpaPiSP::applyBlackLevel(const BlackLevelStatus *blackLevelStatus, pisp_be_global_config &global)
{
	...
	bla.black_level_r = bla.black_level_gr = bla.black_level_gb = bla.black_level_b = 4096;
...
void IpaPiSP::applyLensShading(const AlscStatus *alscStatus, pisp_be_global_config &global)
{
	pisp_be_lsc_extra lscExtra = {};
	pisp_be_lsc_config lsc = {};
	double rgb[3][NumLscVertexes][NumLscVertexes] = {};

	resampleTable(&rgb[0][0][0], NumLscVertexes, NumLscVertexes, alscStatus->r.data(), NumLscCells, NumLscCells);
	resampleTable(&rgb[1][0][0], NumLscVertexes, NumLscVertexes, alscStatus->g.data(), NumLscCells, NumLscCells);
	resampleTable(&rgb[2][0][0], NumLscVertexes, NumLscVertexes, alscStatus->b.data(), NumLscCells, NumLscCells);
...
void IpaPiSP::applyTdn(const TdnStatus *tdnStatus, const DeviceStatus *deviceStatus, pisp_be_global_config &global)
{
...
void IpaPiSP::applySaturation(const SaturationStatus *saturationStatus, pisp_be_global_config &global)
{
...
void IpaPiSP::applySharpen(const SharpenStatus *sharpenStatus, pisp_be_global_config &global)
{
	...
	sharpen.threshold_offset3 = clampField(sharpen.threshold_offset3 * sharpenStatus->threshold, 16);
	sharpen.threshold_offset4 = clampField(sharpen.threshold_offset4 * sharpenStatus->threshold, 16);
	sharpen.threshold_slope0 = clampField(sharpen.threshold_slope0 * sharpenStatus->threshold, 12);
	sharpen.threshold_slope1 = clampField(sharpen.threshold_slope1 * sharpenStatus->threshold, 12);
	sharpen.threshold_slope2 = clampField(sharpen.threshold_slope2 * sharpenStatus->threshold, 12);
	sharpen.threshold_slope3 = clampField(sharpen.threshold_slope3 * sharpenStatus->threshold, 12);
	sharpen.threshold_slope4 = clampField(sharpen.threshold_slope4 * sharpenStatus->threshold, 12);
...
void IpaPiSP::applyHdr(const HdrStatus *hdrStatus, const DeviceStatus *deviceStatus, pisp_be_global_config &global)
{
	utils::Duration exposure = deviceStatus->shutterSpeed * deviceStatus->analogueGain;
...
void IpaPiSP::setStatsAndDebin()
{
	...
	pisp_fe_rgby_config rgby = {};
	rgby.gain_r = rgby.gain_b = clampField(0.25, 14, 10);
	rgby.gain_g = clampField(0.5, 14, 10);
	fe_->SetRGBY(rgby);

	pisp_fe_global_config feGlobal;
	fe_->GetGlobal(feGlobal);
	feGlobal.enables |= PISP_FE_ENABLE_AWB_STATS + PISP_FE_ENABLE_AGC_STATS +
			    PISP_FE_ENABLE_RGBY + PISP_FE_ENABLE_CDAF_STATS;
